function __fzf_hg
  ...
  --bind "ctrl-s:execute(jf s -n -r (echo {} | cut -c -9))+execute(hg show --stat (echo {} | cut -c -9))+abort" \
  --bind "ctrl-t:execute(hg rebase -s (pbpaste) -d (echo {} | cut -c -9))+reload($FZF_DEFAULT_COMMAND)+refresh-preview+clear-query" \
  --bind "ctrl-v:execute(hg show --stat (echo {} | cut -c -9) | grep https | sed 's/.*https/https/' | xargs open)+clear-query" \
