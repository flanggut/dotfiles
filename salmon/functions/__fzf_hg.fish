function __fzf_hg
    ...
    FZF_DEFAULT_COMMAND="hg sl --color always 2>/dev/null > ~/.cache/fzf_hg_smartlog && cat ~/.cache/fzf_hg_smartlog | sed -e 's/^[\|\/\ ╷│├╯╭─╯]*//' | sed -e 's/[0-9]*:[0-9]*//' | sed -e 's/at\ *//' | sed -e 's/D[0-9]\{7,\}//' | egrep -A 1 '(^(o|\@)\ )' | sed '/^--/d' | paste - - | cut -c 4- | sed -e 's/flanggut//' | sed -e 's/[[:blank:]]\{1,\}/ /g'" fzf \
        ...
        --bind "ctrl-p:execute(hg rebase -s (echo {} | cut -c -9) -d (pbpaste) )+reload($FZF_DEFAULT_COMMAND)+refresh-preview+clear-query" \
