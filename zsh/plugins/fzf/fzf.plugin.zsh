...
function __fzf_hg() {
    ...
    FZF_DEFAULT_COMMAND="cat ~/.cache/fzf_hg_list" SELF='source ${ZDOTDIR:-$HOME}/.zsh_plugins.zsh  && __fzf_hg' fzf \
        ...
        --bind 'ctrl-e:execute-silent(hg hide $(echo {} | cut -c -9))+become(eval $SELF)' \
        --bind 'ctrl-u:execute(hg up $(echo {} | cut -c -9))+become(eval $SELF)' \
        --bind 'ctrl-g:execute(hg graft $(echo {} | cut -c -9))+become(eval $SELF)' \
        --bind 'ctrl-h:execute(hg rebase -s $(echo {} | cut -c -9) -d .)+become(eval $SELF)' \
        --bind 'ctrl-p:execute(hg rebase -s $(echo {} | cut -c -9) -d $(pbpaste))+become(eval $SELF)' \
        --bind 'ctrl-r:execute(hg rebase -s $(echo {} | cut -c -9) -d stable)+become(eval $SELF)' \
        ...
        --bind 'ctrl-t:execute(hg rebase -s $(pbpaste) -d $(echo {} | cut -c -9))+become(eval $SELF)' \
